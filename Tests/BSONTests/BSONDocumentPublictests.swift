...
final class BSONDocumentPublicTests: XCTestCase {
    
    static var allTests : [(String, (BSONDocumentPublicTests) -> () throws -> Void)] {
        return [
...
//            ("testDotSyntax", testDotSyntax),
    ...
    let kittenDocument: BSONDocument = [
        "doubleTest": 0.04,
        "stringTest": "foo",
        "documentTest": [
            "documentSubDoubleTest": 13.37,
            "subArray": ["henk", "fred", "kaas", "goudvis"] as BSONDocument
        ] as BSONDocument,
        ...
        "nothing": Value.null,
        "data": Value.binary(subtype: BinarySubtype.generic, data: [34,34,34,34,34]),
        "boolFalse": false,
        "boolTrue": true,
        "timestamp": Value.timestamp(stamp: 2000, increment: 8),
        "regex": Value.regularExpression(pattern: "[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,}", options: "b"),
        "minKey": Value.minKey,
        "maxKey": Value.maxKey
    ]
    
    func validateAgainstKitten(_ document: BSONDocument) {
        ...
        for key in kittenDocument.keys {
            XCTAssertEqual(kittenDocument[key]?.makeBsonValue(), document[key]?.makeBsonValue())
        }
    }
    ...
    func testAppendingContents() {
        var doc = [1, 2, 3, 4, 5] as BSONDocument
...
//        XCTAssertEqual(document["stringTest"], Value.string("foo"))
    ...
    func testArrayRelatedFunctions() {
        let document: BSONDocument = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"]
        XCTAssertTrue(document.validatesAsArray())
        XCTAssertEqual(document.count, 26)
        
        let date = Date()
        let arrayDoc: BSONDocument = ["kaas", "sap", "saus", 55, "fred", ["subDocument": true] as BSONDocument, 44.3, ~date]
    ...
    func testMultipleDocumentsInitialization() {
        let doc1: BSONDocument = ["kaas", "sap", "saus"]
        let doc2 = kittenDocument
        let doc3: BSONDocument = ["hoi": "test", "3": 24]
        
        let data = doc1.bytes + doc2.bytes + doc3.bytes
        
        let singleDoc = BSONDocument(data: data)
        XCTAssertEqual(singleDoc.bytes, doc1.bytes)
        
        let multipleDocs = [BSONDocument](bsonBytes: data)
    ...
    func testInitFromFoundationData() {
        ...
        let document = BSONDocument(data: data)
        XCTAssertEqual(document.bytes, kittenDocument.bytes)
    }
    ...
    func testSerialization() {
        ...
        let documents = [BSONDocument](bsonBytes: originalBinary)
    ...
    func testValidation() {
        ...
        XCTAssertFalse(BSONDocument(data: [0,0,0,0,0]).validate())
        XCTAssertFalse(BSONDocument(data: [4,0,4,0,6,4,32,43,3,2,2,5,6,63]).validate())
        
        let documents0 = [BSONDocument](bsonBytes: [5,0,0,0,0,0,0,0,0,0])
        let documents1 = [BSONDocument](bsonBytes: [5,0,0,0,0,6,0,0,0,0])
        ...
        let containsInvalidDocument = [BSONDocument](bsonBytes: documentsBytes)
        let containsValidDocuments = [BSONDocument](bsonBytes: documentsBytes, validating: true)
        
        XCTAssertEqual(containsInvalidDocument.count, 2)
        XCTAssertEqual(containsValidDocuments.count, 1)
        
        let invalidTypeDocument = BSONDocument(data: [16,0,0,0,80,40,40,40,40,40,0,6,0,0,0,0,40,40,40,40,40,0,0])
...
//        XCTAssertEqual(document["documentTest"][0], ~13.37)
        ...
        XCTAssertEqual(document[document.count] as? Value, nil)
...
//        let recursiveDocument: BSONDocument = [
//            "henk": [
//                "fred": [
//                    "bob": [
//                        "piet": [
...
//        XCTAssertEqual(recursiveDocument["henk"]["fred", "bob", "piet"]["klaas"].int, 3)
    ...
    func testExtendedJSON() throws {
        ...
        let simpleDocument = try BSONDocument(extendedJSON: simpleJson)
        XCTAssertEqual(simpleDocument["kaas"] as? Double, 4.2)
        
        
        let kittenJSON = kittenDocument.makeExtendedJSON()
        
        let otherDocument = try BSONDocument(extendedJSON: kittenJSON)
...
//    }
    ...
    func testDocumentFlattening() {
        let correctFlatKitten: BSONDocument = [
            "doubleTest": 0.04,
            "stringTest": "foo",
            ...
            "nonRandomObjectId": try! ~ObjectId("0123456789ABCDEF01234567"),
            "currentTime": Date(timeIntervalSince1970: Double(1453589266)),
            "cool32bitNumber": Value.int32(9001),
            "cool64bitNumber": 21312153,
    ...
    func testTypeChecking() {
        ...
        XCTAssertEqual(kittenDocument.type(atKey: "doubleTest"), .double)
        XCTAssertEqual(kittenDocument.type(atKey: "stringTest"), .string)
        XCTAssertEqual(kittenDocument.type(atKey: "documentTest"), .document)
//        XCTAssertEqual(kittenDocument["documentTest"].documentValue?.type(at: "subArray"), .arrayDocument)
        XCTAssertEqual(kittenDocument.type(atKey: "nonRandomObjectId"), .objectId)
        XCTAssertEqual(kittenDocument.type(atKey: "bob"), nil)
        XCTAssertEqual(kittenDocument.type(atKey: "piet"), nil)
        XCTAssertEqual(kittenDocument.type(atKey: "kenk"), nil)
    }
}
