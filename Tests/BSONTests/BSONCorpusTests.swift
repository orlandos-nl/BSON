...
final class BSONCorpusTests: XCTestCase {
    func testTopLevel() {
        ...
        XCTAssertFalse(Document(bytes: [0x01,0x00,0x00,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x04,0x00,0x00,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0x01]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0xFF]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0x70]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00,0xDE,0xAD,0xBE,0xEF]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x13,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x11,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x07,0x00,0x00,0x00,0x00,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x07,0x00,0x00,0x00,0x80,0x00,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F]).validate().valid)
    }
//
//    func testTimestamp() {
...
//        
//        XCTAssertFalse(Document(bytes: [0x0f,0x00,0x00,0x00,0x11,0x61,0x00,0x2A,0x00,0x00,0x00,0x15,0xCD,0x5B,0x00]).validate().valid)
//    }
...
//        
//        XCTAssertFalse(Document(bytes: [0x0F,0x00,0x00,0x00,0x0B,0x61,0x00,0x61,0x00,0x63,0x00,0x69,0x6D,0x00,0x00]).validate().valid)
//        XCTAssertFalse(Document(bytes: [0x10,0x00,0x00,0x00,0x0B,0x61,0x00,0x61,0x62,0x63,0x00,0x69,0x00,0x6D,0x00,0x00]).validate().valid)
//    }
//    
    func testObjectId() throws {
        ...
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x07,0x61,0x00,0x56,0xE1,0xFC,0x72,0xE0,0xC9,0x17,0xE9,0xC4,0x71]).validate().valid)
    }
//
//    func testNull() {
//        let doc = Document(bytes: [0x08,0x00,0x00,0x00,0x0A,0x61,0x00,0x00])
//        XCTAssert(doc.validate().valid)
...
//        
//        XCTAssertFalse(Document(bytes: [0x14,0x00,0x00,0x00,0x04,0x61,0x00,0x0D,0x00,0x00,0x00,0x10,0x30,0x00,0x0A,0x00,0x00,0x00,0x00,0x00]).validate().valid)
//        XCTAssertFalse(Document(bytes: [0x14,0x00,0x00,0x00,0x04,0x61,0x00,0x0B,0x00,0x00,0x00,0x10,0x30,0x00,0x0A,0x00,0x00,0x00,0x00,0x00]).validate().valid)
//        XCTAssertFalse(Document(bytes: [0x1A,0x00,0x00,0x00,0x04,0x66,0x6F,0x6F,0x00,0x10,0x00,0x00,0x00,0x02,0x30,0x00,0x05,0x00,0x00,0x00,0x62,0x61,0x7A,0x00,0x00,0x00]).validate().valid)
//    }
    ...
    func testBoolean() {
        ...
        XCTAssertFalse(Document(bytes: [0x09,0x00,0x00,0x00,0x08,0x62,0x00,0x02,0x00]).validate().valid)
        XCTAssertFalse(Document(bytes: [0x09,0x00,0x00,0x00,0x08,0x62,0x00,0xFF,0x00]).validate().valid)
    }
    ...
    func testDouble() {
        ...
        XCTAssert(doc8.validate().valid)
        XCTAssert(doc9.validate().valid)
        XCTAssert(doc10.validate().valid)
        XCTAssert(doc11.validate().valid)
        ...
        XCTAssertFalse(Document(bytes: [0x0B,0x00,0x00,0x00,0x01,0x64,0x00,0x00,0x00,0xF0,0x3F,0x00]).validate().valid)
    }
