...
final class BSONCorpusTests: XCTestCase {
    func testTopLevel() {
        ...
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0x01]).validate())
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0xFF]).validate())
        XCTAssertFalse(Document(bytes: [0x05,0x00,0x00,0x00,0x70]).validate())
        XCTAssertFalse(Document(bytes: [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72]).validate())
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00,0xDE,0xAD,0xBE,0xEF]).validate())
        XCTAssertFalse(Document(bytes: [0x13,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x11,0x00,0x00,0x00,0x02,0x66,0x6F,0x6F,0x00,0x04,0x00,0x00,0x00,0x62,0x61,0x72,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x07,0x00,0x00,0x00,0x00,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x07,0x00,0x00,0x00,0x80,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x66,0x6F]).validate())
    }
//
//    func testTimestamp() {
...
//        
//        XCTAssertFalse(Document(bytes: [0x0f,0x00,0x00,0x00,0x11,0x61,0x00,0x2A,0x00,0x00,0x00,0x15,0xCD,0x5B,0x00]).validate())
//    }
    ...
    func testString() {
        ...
        XCTAssertFalse(Document(bytes: [0x0C,0x00,0x00,0x00,0x02,0x61,0x00,0x00,0x00,0x00,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x0C,0x00,0x00,0x00,0x02,0x61,0x00,0xFF,0xFF,0xFF,0xFF,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x10,0x00,0x00,0x00,0x02,0x61,0x00,0x05,0x00,0x00,0x00,0x62,0x00,0x62,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x02,0x00,0xFF,0xFF,0xFF,0x00,0x66,0x6F,0x6F,0x62,0x61,0x72,0x00,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x10,0x00,0x00,0x00,0x02,0x61,0x00,0x04,0x00,0x00,0x00,0x61,0x62,0x63,0xFF,0x00]).validate())
        XCTAssertFalse(Document(bytes: [0x0E,0x00,0x00,0x00,0x02,0x61,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00]).validate())
        
        // Invalid UTF-8 test is different from the mongodb spec, we think this is swift's problem
        XCTAssertNil(Document(bytes: [0x0E,0x00,0x00,0x00,0x02,0x61,0x00,0x02,0x00,0x00,0x00,0xE9,0x00,0x00])["a"] as? String)
    }
...
//        
//        XCTAssertFalse(Document(bytes: [0x0F,0x00,0x00,0x00,0x0B,0x61,0x00,0x61,0x00,0x63,0x00,0x69,0x6D,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x10,0x00,0x00,0x00,0x0B,0x61,0x00,0x61,0x62,0x63,0x00,0x69,0x00,0x6D,0x00,0x00]).validate())
//    }
//    
    func testObjectId() throws {
        ...
        XCTAssertFalse(Document(bytes: [0x12,0x00,0x00,0x00,0x07,0x61,0x00,0x56,0xE1,0xFC,0x72,0xE0,0xC9,0x17,0xE9,0xC4,0x71]).validate())
    }
//
//    func testNull() {
...
//    }
    ...
    func testInt64() {
        ...
        XCTAssertEqual(doc0["a"] as? Int64, -9223372036854775808)
        XCTAssertEqual(doc1["a"] as? Int64, 9223372036854775807)
        XCTAssertEqual(doc2["a"] as? Int64, -1)
        XCTAssertEqual(doc3["a"] as? Int64, 0)
        XCTAssertEqual(doc4["a"] as? Int64, 1)
    }
...
//        
//        XCTAssertFalse(Document(bytes: [0x14,0x00,0x00,0x00,0x04,0x61,0x00,0x0D,0x00,0x00,0x00,0x10,0x30,0x00,0x0A,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x14,0x00,0x00,0x00,0x04,0x61,0x00,0x0B,0x00,0x00,0x00,0x10,0x30,0x00,0x0A,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x1A,0x00,0x00,0x00,0x04,0x66,0x6F,0x6F,0x00,0x10,0x00,0x00,0x00,0x02,0x30,0x00,0x05,0x00,0x00,0x00,0x62,0x61,0x7A,0x00,0x00,0x00]).validate())
//    }
    ...
    func testDouble() {
        ...
        XCTAssertFalse(Document(bytes: [0x0B,0x00,0x00,0x00,0x01,0x64,0x00,0x00,0x00,0xF0,0x3F,0x00]).validate())
    }
...
//        
//        XCTAssertFalse(Document(bytes: [0x18,0x00,0x00,0x00,0x03,0x66,0x6F,0x6F,0x00,0x0F,0x00,0x00,0x00,0x10,0x62,0x61,0x72,0x00,0xFF,0xFF,0xFF,0x7F,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x15,0x00,0x00,0x00,0x03,0x66,0x6F,0x6F,0x00,0x0A,0x00,0x00,0x00,0x08,0x62,0x61,0x72,0x00,0x01,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x1C,0x00,0x00,0x00,0x03,0x66,0x6F,0x6F,0x00,0x12,0x00,0x00,0x00,0x02,0x62,0x61,0x72,0x00,0x05,0x00,0x00,0x00,0x62,0x61,0x7A,0x00,0x00,0x00]).validate())
//    }
//    
//    func testDateTime() {
...
//        
//        XCTAssertFalse(Document(bytes: [0x0C,0x00,0x00,0x00,0x09,0x61,0x00,0x12,0x34,0x56,0x78,0x00]).validate())
//    }
//    
//    func testBinary() {
...
//        
//        XCTAssertFalse(Document(bytes: [0x1D,0x00,0x00,0x00,0x05,0x78,0x00,0xFF,0x00,0x00,0x00,0x05,0x73,0xFF,0xD2,0x64,0x44,0xB3,0x4C,0x69,0x90,0xE8,0xE7,0xD1,0xDF,0xC0,0x35,0xD4,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x0D,0x00,0x00,0x00,0x05,0x78,0x00,0xFF,0xFF,0xFF,0xFF,0x00,0x00]).validate())
//        
//        // TODO: Support old code?
////        XCTAssertFalse(Document(bytes: [0x13,0x00,0x00,0x00,0x05,0x78,0x00,0x06,0x00,0x00,0x00,0x02,0x03,0x00,0x00,0x00,0xFF,0xFF,0x00]).validate())
////        XCTAssertFalse(Document(bytes: [0x13,0x00,0x00,0x00,0x05,0x78,0x00,0x06,0x00,0x00,0x00,0x02,0x01,0x00,0x00,0x00,0xFF,0xFF,0x00]).validate())
////        XCTAssertFalse(Document(bytes: [0x13,0x00,0x00,0x00,0x05,0x78,0x00,0x06,0x00,0x00,0x00,0x02,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00]).validate())
//    }
//    
//    func testCode() {
...
////        XCTAssertFalse(Document(bytes: [0x0E,0x00,0x00,0x00,0x02,0x61,0x00,0x02,0x00,0x00,0x00,0xE9,0x00,0x00]).validate())
//    }
//    
...
//        
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x00,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0xFF,0xFF,0xFF,0xFF,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x16,0x00,0x00,0x00,0x0F,0x61,0x00,0x0D,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x1F,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x21,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0xFF,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//// TODO: Fix validation       XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x20,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//// TODO: Fix validation       XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x20,0x00,0x00,0x00,0x06,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x21,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0xFF,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        // TODO: Fix validation       XCTAssertFalse(Document(bytes: [0x28,0x00,0x00,0x00,0x0F,0x61,0x00,0x20,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x00,0x13,0x00,0x00,0x00,0x10,0x78,0x00,0x01,0x00,0x00,0x00,0x10,0x79,0x00,0x01,0x00,0x00,0x00,0x00,0x00]).validate())
//        
